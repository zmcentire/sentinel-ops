...
const evaluatorWorker = new _bullmq.Worker('evaluations', async (job)=>{
    const { endpointId } = job.data;
    // Fetch all active rules for this endpoint
    const { rows: rules } = await _db.pool.query(`SELECT * FROM alert_rules WHERE endpoint_id = $1 AND active = true`, [
        endpointId
    ]);
    for (const rule of rules){
        ...
        const value = parseFloat(metrics[rule.metric]);
        const violated = rule.operator === 'gt' ? value > rule.threshold : rule.operator === 'lt' ? value < rule.threshold : false;
        if (!violated) continue;
        // Check cooldown — is there already an open/acknowledged incident
        // opened within the cooldown window for this endpoint?
        const { rows: recentIncidents } = await _db.pool.query(`SELECT id FROM incidents
         WHERE endpoint_id = $1
           AND status != 'resolved'
        ...
        const body = `Auto-opened by alert rule ${rule.id}. ` + `${rule.metric}=${value.toFixed(1)} ` + `(threshold: ${rule.operator} ${rule.threshold})`;
        const { rows: [incident] } = await _db.pool.query(`INSERT INTO incidents (endpoint_id, severity, title, body)
         VALUES ($1, $2, $3, $4)
         RETURNING *`, [
            endpointId,
            rule.severity,
        ...
        await _queues.notificationsQueue.add('notify', {
            incident,
            rule,
            currentValue: value
        });
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function buildTitle(rule, value) {
    const metricLabel = {
        latency_p99: 'P99 latency',
        uptime_pct: 'Uptime',
        error_rate: 'Error rate'
    };
    const label = metricLabel[rule.metric] ?? String(rule.metric);
    const direction = rule.operator === 'gt' ? 'above' : 'below';
    const unit = String(rule.metric).includes('pct') || String(rule.metric).includes('rate') ? '%' : 'ms';
