...
const notifierWorker = new Worker<NotifyJobData>(
  'notifications',
  async (job) => {
...
async function sendSms(incident: Incident, dashUrl: string): Promise<void> {
...
async function sendEmail(incident: Incident, dashUrl: string): Promise<void> {
  const sev      = incident.severity.toUpperCase();
  const sevColor =
    incident.severity === 'p1' ? '#c0392b' :
    incident.severity === 'p2' ? '#d4860a' : '#2a9d5c';
